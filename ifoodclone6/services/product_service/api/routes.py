...
Routes - API Layer
==================
Definição de rotas para product_service.
"""
...
router = APIRouter(prefix="/api/product_service", tags=["product_service"])
...
async def get_product(
    ...
    if not entity:
        raise HTTPException(status_code=404, detail=f"{Product} não encontrado")
    return ProductSchema.from_orm(entity)
