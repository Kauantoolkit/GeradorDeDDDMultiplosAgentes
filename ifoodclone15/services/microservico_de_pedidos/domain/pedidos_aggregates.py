...
=========================
Agregados para o domínio pedidos.
Agregado é um cluster de entidades e value objects com raiz (root entity).
"""

from uuid import UUID
from .pedidos_entities import PedidosEntity


class PedidosAggregate:
    """
    Agregado raiz para o domínio pedidos.
    Controla invariantes de negócio e transações.
    """
    
    def __init__(self, root: PedidosEntity):
        self._root = root
        self._entities = [root]
    
    @property
    def root(self) -> PedidosEntity:
        return self._root
