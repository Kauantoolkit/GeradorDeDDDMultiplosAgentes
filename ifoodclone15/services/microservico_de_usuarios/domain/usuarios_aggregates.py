...
=========================
Agregados para o domínio usuarios.
Agregado é um cluster de entidades e value objects com raiz (root entity).
"""

from uuid import UUID
from .usuarios_entities import UsuariosEntity


class UsuariosAggregate:
    """
    Agregado raiz para o domínio usuarios.
    Controla invariantes de negócio e transações.
    """
    
    def __init__(self, root: UsuariosEntity):
        self._root = root
        self._entities = [root]
    
    @property
    def root(self) -> UsuariosEntity:
        return self._root
