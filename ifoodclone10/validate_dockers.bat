...
echo.
echo ============================================
echo  Verificando reviews
echo ============================================
for /f "tokens=*" %%p in ('docker-compose port reviews 8000 2^>nul') do set SERVICE_PORT=%%p
if not defined SERVICE_PORT (
    echo WARNING: nao foi possivel resolver porta do servico reviews
) else (
    curl -s http://!SERVICE_PORT!/health || echo "WARNING: reviews nao responde em http://!SERVICE_PORT!/health"
)
set SERVICE_PORT=
