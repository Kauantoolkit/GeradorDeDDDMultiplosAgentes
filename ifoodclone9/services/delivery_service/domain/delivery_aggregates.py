...
=========================
Agregados para o domínio delivery.
Agregado é um cluster de entidades e value objects com raiz (root entity).
"""

from uuid import UUID
from .delivery_entities import Delivery


class DeliveryAggregate:
    """
    Agregado raiz para o domínio delivery.
    Controla invariantes de negócio e transações.
    """
    
    def __init__(self, root: Delivery):
        self._root = root
        self._entities = [root]
    
    @property
    def root(self) -> Delivery:
        return self._root
