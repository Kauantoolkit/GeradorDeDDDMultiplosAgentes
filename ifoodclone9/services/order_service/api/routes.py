...
Routes - API Layer
==================
Definição de rotas para order_service.
"""
...
router = APIRouter(prefix="/api/order_service", tags=["order_service"])
...
async def get_order(
    ...
    if not entity:
        raise HTTPException(status_code=404, detail=f"{Order} não encontrado")
    return OrderSchema.from_orm(entity)
