...
echo.
echo ============================================
echo  Verificando payment_service
echo ============================================
for /f "tokens=*" %%p in ('docker-compose port payment_service 8000 2^>nul') do set SERVICE_PORT=%%p
if not defined SERVICE_PORT (
    echo WARNING: nao foi possivel resolver porta do servico payment_service
) else (
    curl -s http://!SERVICE_PORT!/health || echo "WARNING: payment_service nao responde em http://!SERVICE_PORT!/health"
)
set SERVICE_PORT=
