...
async def list_tasks():
    ...
    # Você pode implementar um gerenciador de tasks
    return {
        "tasks": [],
        "timestamp": datetime.now().isoformat()
    }
...
async def execute_generation(
    ...
    Esta função é executada em background e envia eventos
    para o cliente durante toda a execução.
    """
    ...
    async def send_update(event_type: str, data: dict):
        ...
        # Envia para o cliente específico (se conectado)
        # Por ora, faz broadcast para todos
        await manager.broadcast(message)
        
        # Também tenta encontrar a conexão específica
        # (pode ser implementado com um dicionário de tasks)
